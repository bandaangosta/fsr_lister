...
## Development environment and release process
 ...
 - create virtualenv with Flask and FSR Lister installed into it (latter is installed in
...
install it (`pip install fsr_lister.tar.gz`), ensure that configuration file exists and
`FSR_LISTER_SETTINGS` environment variable points to it, ensure that user has access to the
...
While in project folder, where Dockerfiles reside, run:
    ...
    docker build -t fsr_lister -f Dockerfile.xxx .
    docker run -it --rm -p 80:80 --name my_fsr_lister fsr_lister

If tested OK, you can run the app in background:

   docker run --detach -p 80:80 --restart unless-stopped --name my_fsr_lister fsr_lister   (to run on reboot)
